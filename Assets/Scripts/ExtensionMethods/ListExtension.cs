...
public static class ListExtension
{
	...
	/// </summary>
	public static void MaintainDescending<T>(this List<T> list, T element) where T : IComparable<T>
	{
		Organize(list, element, (i, j) => list[i].CompareTo(list[j]) > 0);
	}
	...
	/// </summary>
	public static void MaintainAscending<T>(this List<T> list, T element) where T : IComparable<T>
	{
		Organize(list, element, (int i, int i1) => { return list[i].CompareTo(list[i1]) < 0; });
	}

	private static void Organize<T>(List<T> list, T element, Func<int, int, bool> predicate)
	{
		list.Add(element);
